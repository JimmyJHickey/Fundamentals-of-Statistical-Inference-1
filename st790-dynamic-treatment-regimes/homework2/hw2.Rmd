...
## f
There are some clear errors in my calculations. Most noticeably, my estimates are negative! Further, I had to go from 100 bootstrap reptitions (which took about an hour bootstrap call) to 10 reptitions (which took a minute or two each). 

Based on my testing, I think that the inefficiencies are coming from the creation of my design matrices. I think allocating a fixed amount of memory for them at the beginning would improve performance. My current solution is to start them as NULL and add rows along the way
...
## d
The results for (b) and (c) are very similar to one another in both estimates and standard errors. It is hard to compare them to the g-computation algorithm while I still have bugs.